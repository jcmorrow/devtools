...
const Button = ({ style, icon }: { style: CSSProperties; icon: string }) => (
  <button
...
function ToggleButton({
  ...
  const [isMetaActive, setIsMetaActive] = useState(event.metaKey);
  const [isShiftActive, setIsShiftActive] = useState(event.shiftKey);
  ...
  return (
    <div
      className="absolute z-50 flex -translate-y-1/2 transform flex-row space-x-px"
      style={{ top: `${(1 / 2) * height}px` }}
    >
      <Button
        style={style}
        icon={isMetaActive ? (isShiftActive ? "navigate_before" : "navigate_next") : icon}
      />
    </div>
  );
}

type ToggleWidgetButtonProps = PropsFromRedux & { editor: any; event: MouseEvent };
